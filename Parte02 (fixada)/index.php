...
<head>
	...
	<?php
			...
			$sql = "SELECT palavra, dica FROM palavras ORDER BY rand() LIMIT 15";
			$stmt = $conn->query($sql); //statement para consulta (query)
			$palavras = array();
			$dicas = array();
			$i = 0;
			if ($stmt!=false) {
			...
			//$dicas = json_encode($dicas); //NÃO CONSEGUIMOS CONVERTER AS DICAS, PARA USA-LAS COMO VARIAVEL NO JAVASCRIPT
...
<body onload="inicializar()">
	...
	<div id="dica">
		...
		<p>dicas aqui(Algumas Não Estão No Jogo)</p>
		<?php 
		while($i >= 0){
			echo"|".($dicas[$i])."| <br>";
			$i--;
